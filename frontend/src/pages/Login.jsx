...
export default function Login() {
  ...
  return <section className="auth-shell">
    ...
    <div className="auth-panel"><div className="auth-panel-inner"><span className="mobile-logo">SR</span><p className="eyebrow">Secure portal</p><h2>Welcome back</h2><p>Sign in or create a student account with Google.</p>{error && <div className="message error">{error}</div>}<GoogleSignIn onError={setError} /><form autoComplete="off" onSubmit={submit}><label>Email address<input type="email" autoComplete="off" required placeholder="you@school.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label>Password<input type="password" autoComplete="new-password" required placeholder="Enter your password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label><Link className="forgot-password-link" to="/forgot-password">Forgot password?</Link><button className="wide-button" disabled={loading}>{loading ? 'Signing you in...' : 'Sign in to your account'}<Icon name="arrow" size={18} /></button></form><p className="auth-switch">New to the portal? <Link to="/register" state={{ fromLogin: true }}>Create with email instead</Link></p><p className="security-note"><Icon name="shield" size={16} /> Your credentials are encrypted and protected.</p></div></div>
  </section>
}
